    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑代驾司机收藏</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">代驾司机收藏管理</li>
                        <li class="breadcrumb-item active">编辑代驾司机收藏</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">代驾司机收藏信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 daijiaCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="daijiaCollectionTypesSelect" name="daijiaCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    var tableName = "daijiaCollection";
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var daijiaCollectionTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var yonghuOptions = [];
    var daijiaOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("daijiaCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('adddaijiaCollection');
    ...
    // 查询列表
        <!-- 查询当前表的所有列表 -->
        function daijiaCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=daijia_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    daijiaCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationDaijiacollectiontypesSelect(){
            var daijiaCollectionTypesSelect = document.getElementById('daijiaCollectionTypesSelect');
            if(daijiaCollectionTypesSelect != null && daijiaCollectionTypesOptions != null  && daijiaCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < daijiaCollectionTypesOptions.length; i++) {
                    daijiaCollectionTypesSelect.add(new Option(daijiaCollectionTypesOptions[i].indexName,daijiaCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(daijiaCollectionTypesSelect != null && daijiaCollectionTypesOptions != null  && daijiaCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < daijiaCollectionTypesOptions.length; i++) {
                if (daijiaCollectionTypesOptions[i].codeIndex == ruleForm.daijiaCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        daijiaCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                daijiaId: "required",
                yonghuId: "required",
                daijiaCollectionTypes: "required",
            },
            messages: {
                daijiaId: "代驾司机不能为空",
                yonghuId: "用户不能为空",
                daijiaCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var adddaijiaCollection = window.sessionStorage.getItem("adddaijiaCollection");
        if (adddaijiaCollection != null && adddaijiaCollection != "" && adddaijiaCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("daijiaCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationDaijiacollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
