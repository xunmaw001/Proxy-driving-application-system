        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">代驾司机收藏展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="daijiaUuidNumber" id="daijiaUuidNumber" placeholder="工号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="daijiaName" id="daijiaName" placeholder="代驾司机姓名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteDaijiaCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../daijia/detail.jsp?id='+item.daijiaId , item.daijiaDelete == null?'':item.daijiaDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.daijiaPhoto?item.daijiaPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.daijiaName}}
                                        <span v-if="item.daijiaNewMoney"> - ￥{{item.daijiaNewMoney}}</span>
                                        <span v-else-if="item.daijiaTypes"> - {{item.daijiaValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            daijiaCollectionTypesList : [],
        // 级联表字典表数据
            daijiaCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteDaijiaCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('daijiaCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function daijiaCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=daijia_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.daijiaCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            if (jquery('#daijiaUuidNumber').val() != null && jquery('#daijiaUuidNumber').val() != "" && jquery('#daijiaUuidNumber').val() != "null" ) {
                param['daijiaUuidNumber'] = jquery('#daijiaUuidNumber').val();
            }
                     
            if (jquery('#daijiaName').val() != null && jquery('#daijiaName').val() != "" && jquery('#daijiaName').val() != "null" ) {
                param['daijiaName'] = jquery('#daijiaName').val();
            }
                                                                                                                                                                                                        
            // 当前表
                                                
            //收藏只能查类型为1的
            param['daijiaCollectionTypes'] = 1;

            // 获取列表数据
            http.request('daijiaCollection/page', 'get', param, function(res) {
