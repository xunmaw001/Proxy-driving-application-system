		...
		<script>
			var vue = new Vue({
				...
				methods: {
					...
					,payClick() {
						let data={
                            daijiaId: localStorage.getItem('userid'),
                            yonghuId: localStorage.getItem('userid'),
                            daijiaOrderPaymentTypes: vue.daijiaOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`daijiaOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../daijiaOrder/list.jsp';
							}
                       		 window.location.href='../daijiaOrder/list.jsp';
					    });
					}
